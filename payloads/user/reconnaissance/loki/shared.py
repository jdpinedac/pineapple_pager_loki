#shared.py
# Description:
# Shared resources and data for Loki on WiFi Pineapple Pager.
...
class SharedData:
    ...
    def initialize_paths(self):
        ...
        loki_theme_dir = os.path.join(self.currentdir, 'themes', 'loki')
        self.picdir = os.path.join(loki_theme_dir, 'images')
        self.fontdir = os.path.join(self.resourcesdir, 'fonts')
        self.commentsdir = os.path.join(loki_theme_dir, 'comments')

        # Directories under picdir
        self.statuspicdir = os.path.join(self.picdir, 'status')
        self.staticpicdir = self.picdir
    ...
    def initialize_variables(self):
        ...
        self.lokisay = "Hacking away..."
        self.lokiorch_status = "IDLE"
        self.lokistatustext = "IDLE"
        self.lokistatustext2 = "Awakening..."
    ...
    def load_fonts(self):
        ...
        try:
            ...
            self.font_viking_path = os.path.join(self.currentdir, 'themes', 'loki', 'fonts', 'title.TTF')
            self.font_cartoon_path = os.path.join(self.fontdir, 'Arial.ttf')
            self.font_creamy_path = os.path.join(self.fontdir, 'Arial.ttf')
    ...
    def load_images(self):
        ...
        try:
            ...
            self.lokistatusimage_path = None
            ...
            self.image_series = {}
            # Always include IDLE (not an action module, but has animation frames)
            all_statuses = list(self.status_list)
            if "IDLE" not in all_statuses:
                all_statuses.insert(0, "IDLE")
            for status in all_statuses:
                ...
                if os.path.isdir(status_dir):
                    ...
                    for image_name in sorted(os.listdir(status_dir), key=_nat_key):
    ...
    def load_theme(self):
        ...
        theme_name = self.config.get("theme", "loki")
        ...
        if os.path.isdir(theme_status_dir):
            ...
            for status in list(self.image_series.keys()):
                ...
                if os.path.isdir(theme_action_dir):
                    ...
                    def _nat_key(s):
                        return [int(c) if c.isdigit() else c.lower() for c in re.split(r'(\d+)', s)]
                    for image_name in sorted(os.listdir(theme_action_dir), key=_nat_key):
                        if self._is_image(image_name) and re.search(r'\d', image_name):
    ...
    def update_lokistatus(self):
        ...
        try:
            if self.lokiorch_status in self.status_images:
                self.lokistatusimage_path = self.status_images[self.lokiorch_status]
            else:
                logger.warning(f"No image for status {self.lokiorch_status}, using IDLE")
                self.lokistatusimage_path = self.status_images.get('IDLE')

            self.lokistatustext = self.lokiorch_status
        except Exception as e:
            logger.error(f"Error updating loki status: {e}")

    def update_image_randomizer(self):
        """Select the next animation frame for current status."""
        try:
            status = self.lokiorch_status
            ...
            if getattr(self, 'animation_mode', 'random') == 'sequential':
                # Reset to frame 0 only when switching to a different animation
                if frames is not getattr(self, '_last_seq_frames', None):
                    self._frame_index = 0
                    self._last_seq_frames = frames
