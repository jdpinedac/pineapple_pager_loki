...
class SQLBruteforce:
    ...
    def __init__(self, shared_data):
        ...
        logger.debug("SQLConnector initialized.")
    ...
    def execute(self, ip, port, row, status_key):
        ...
        self.shared_data.lokiorch_status = "SQLBruteforce"
...
class SQLConnector:
    ...
    def worker(self, success_flag):
        ...
        while True:
            ...
            try:
                # sql_connect returns (bool, databases) tuple — non-None, so retry logic works
                result = try_connect_with_retries(
                    self.sql_connect, (adresse_ip, user, password),
                    attempt_timeout, max_retries,
                    self.retry_counter, self.retry_lock, logger)

                if result is None:
                    logger.warning(f"SQL brute force aborting for {adresse_ip}: credential timed out {max_retries} times")
                    self.abort_flag.set()
                ...
                with self.retry_lock:
                    if self.retry_counter['total'] >= max_total:
                        logger.warning(f"SQL brute force aborting for {adresse_ip}: {max_total} total retries reached")
                        self.abort_flag.set()
                        drain_queue_safely(self.queue)
                        break

                success, databases = result
                if success:
                    with self.lock:
                        for db in databases:
