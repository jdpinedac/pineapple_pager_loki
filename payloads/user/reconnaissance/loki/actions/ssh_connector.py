...
class SSHBruteforce:
    ...
    def __init__(self, shared_data):
        ...
        logger.debug("SSHConnector initialized.")
    ...
    def execute(self, ip, port, row, status_key):
        ...
        self.shared_data.lokiorch_status = "SSHBruteforce"
...
class SSHConnector:
    ...
    def worker(self, success_flag):
        ...
        Uses per-attempt timeout with retry logic.
        """
        ...
        while True:
            ...
            try:
                result = try_connect_with_retries(
                    self.ssh_connect, (adresse_ip, user, password),
                    attempt_timeout, max_retries,
                    self.retry_counter, self.retry_lock, logger)

                if result is None:
                    # Credential timed out on all retries — host unresponsive
                    logger.warning(f"SSH brute force aborting for {adresse_ip}: credential timed out {max_retries} times")
                    self.abort_flag.set()
                ...
                with self.retry_lock:
                    if self.retry_counter['total'] >= max_total:
                        logger.warning(f"SSH brute force aborting for {adresse_ip}: {max_total} total retries reached")
                        self.abort_flag.set()
                        drain_queue_safely(self.queue)
                        break
